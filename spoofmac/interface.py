...
class LinuxSpoofer(OsSpoofer):
    ...
    def reconnect_wifi(self, device, wifi_name):
        subprocess.check_output = check_output("iwconfig {} essid {}".format(device, wifi_name), shell=True)
...
class WindowsSpoofer(OsSpoofer):
    ...
    def reconnect_wifi(self, device, wifi_name):
        subprocess.check_output = check_output("netsh {} connect {}".format(device, wifi_name), shell=True)
...
class MacSpoofer(OsSpoofer):
    ...
    def reconnect_wifi(self, device, wifi_name):
        subprocess.check_output = check_output("networksetup -setairportnetwork {} {}".format(device, wifi_name), shell=True)
